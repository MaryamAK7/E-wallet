...
import TransactionsForm from "../../Components/TransForm/TransactionsForm";
...
export default function SingleWallet() {
  const {  WalletList, setChosenWallet } = useContext(
    ListWalletContext
  );
  let { name } = useParams();
  ...
  useEffect(() => {
    WalletList.forEach((wal) =>
      wal.walletName === name ? setChosenWallet(wal) : null
    );
